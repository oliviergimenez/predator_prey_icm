...
plot(N,fecV_intrasp,type='l',lwd=3,col='blue',ylab='V fecundity',ylim=c(0,25),xlab='Adult V abundance',main='INTRA-DD\n prey fec vs ad prey abund')
lines(N,fecV_intrasp_est,type='l',lwd=3,col='green')
legend('topright',col=c('blue','green'),legend=c('actual','estimated'),lty=1,lwd=3)

pairs(ipm.prod$BUGSoutput$sims.list$alpha)
parcorplot(ipm.prod,parms = "alpha")

## All possible curves
alpha3 = ipm.prod$BUGSoutput$sims.list$alpha[,3]
alpha4 = ipm.prod$BUGSoutput$sims.list$alpha[,4]
n = length(alpha3)
ndens = 100
N <- seq(10,1000,length=ndens) #density index
surv_juvV_hat = matrix(NA,nrow = n, ncol = ndens)
par(mfrow=c(1,1))
library(scales)
for (i in 1:n){
  for (dens in 1:length(N))
  {
surv_juvV_hat[i,dens] = 1/(1+exp(-(alpha3[i]+ alpha4[i] * N[dens])))
  }
  if (i == 1){plot(N,surv_juvV_hat[i,],type='l',lwd=0.5,col=alpha('black',0.05),ylab='Juvenile V survival',ylim=c(0,0.7),xlim=c(0,400),xlab='P abundance',main='INTER-DD\n juv prey surv vs pred abund')
  }
  else {lines(N,surv_juvV_hat[i,],type='l',lwd=0.5,col=alpha('black',0.05))}
}

# what if there was no correlation between parameters? 
# randomize alpha3 and alpha4

alpha3 = sample(alpha3)
alpha4 = sample(alpha4)
library(scales)
for (i in 1:n){
  for (dens in 1:length(N))
  {
