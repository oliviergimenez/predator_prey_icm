...
for (t in 1:(n.occasions-1)){
   ...
   log(fecP[t]) <- alpha[5] + log(1.0 - exp(-alpha[6] * N1V[t])) # inter-species DD - predator fecundity fn of juvenile prey abundance
...
alpha[6] <- 0.005 # slopes
exp(alpha[5])*(1 - exp(-alpha[6] * N))
...
x = ipm.prod$BUGSoutput$sims.list$alpha[,6]
...
# inter-species DD - predator fecundity fn of juvenile prey abundance
fecP_intersp = exp(alpha[5])*(1-exp(-alpha[6] * N)) 
fecP_intersp_est = exp(alpha_est[5])*(1-exp(-alpha_est[6]*N)) 
