...
for (t in 1:(n.occasions-1)){
   ...
   fecP[t] <- alpha[5] #(1 - exp(-alpha[6] * N1V[t])) # inter-species DD - predator fecundity fn of juvenile prey abundance
...
alpha[5] <- 2 #max fecundity - here a constant value
alpha[6] <- 0.005 # slopes - does not matter because fecundity is set to a constant
alpha[5] #*(1 - exp(-alpha[6] * N)) # how we changed the code
...
for (j in 1:4){
	alpha[2*j-1] ~ dnorm(0,2) # priors on density-dependence parameters
  # sd changed from 1 to 2 because of alpha[5] that is about 5
  alpha[2*j] ~ dnorm(0,0.1) # not on the same scale
}
...
# inter-species DD - predator fecundity fn of juvenile prey abundance
fecP_intersp = alpha[5]*rep(1,length(N)) #*(1-exp(-alpha[6] * N)) 
fecP_intersp_est = alpha_est[5]*rep(1,length(N)) #*(1-exp(-alpha_est[6]*N)) 
...
plot(N,fecP_intersp,type='l',lwd=3,col='blue',ylab='P fecundity',ylim=c(0,5),xlab=' Juv V abundance',main='INTER-DD\n pred fec vs juv prey abund')
lines(N,fecP_intersp_est,type='l',lwd=3,col='green')
legend('topright',col=c('blue','green'),legend=c('actual','estimated'),lty=1,lwd=3)

plot(N,fecV_intrasp,type='l',lwd=3,col='blue',ylab='V fecundity',ylim=c(0,5),xlab='Adult V abundance',main='INTRA-DD\n prey fec vs ad prey abund')
