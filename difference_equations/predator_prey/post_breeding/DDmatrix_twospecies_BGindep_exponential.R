...
### 18/03/2018 Added the exponential function just like in Neubert & Caswell or Bachar's internship 
###  Version with no interspecific interactions
#####################################################################################################################################
...
# lambda[i] is the new parameter in the exponential
#############################################################################################################
...
f=rep(NA,4)
#for (i in 1:3){f[i] = exp(-lambda[i]*N[i])}
f[1] = exp(-lambda[1]*N[2])
f[2] = 1
f[3] = exp(-lambda[3]*N[4])
f[4] = 1 

VR_prey = matrix(c((1-params[5])*params[1]*f[2], params[7]*f[1], 
            params[5]*params[1]*f[2],  params[2]),nrow = 2, ncol = 2)

VR_pred =  matrix(c((1-params[6])*params[3]*f[3], params[8]*f[4], 
             params[6]*params[3]*f[3], params[4]),nrow = 2, ncol = 2)
...
phiA_V = 0.6
# Predator survival 
phiJ_P = 0.7 ## 0.5 to 0/7 
phiA_P = 0.9 ## 0.9 to 0.97
# Transition rate from juvenile to adulthood
gamma_V = 1
gamma_P = 0.5 #1/4 to 1 
# Reproduction (b or pi or f) max rate // only adults reproduce
b_P = 3 # 0.25 (albatross -- one chick every two year, hence 1 female in 4 years) to 7 (artic fox)
b_V = 7 # 5 to 10 (20 females sounds like a max per year, even for a vole...)
...
beta = c(15,30,5,10)
eta = c(-1,-1,-1,1)
### Threshold changed for exponential, since these are defined to half-reduction densities
x_lambda = c(100,100,15,15)
lambda = log(2)/x_lambda
...
params = c(phiJ_V,phiA_V,phiJ_P,phiA_P,gamma_V,gamma_P,b_P,b_V,lambda)
...
pdf(file="TimePlot_BGindepexponential.pdf",width=12,height=8)
par(cex=1.2)
matplot(t(N[,20:tmax]),xlab="Time",ylab="Densities")
matlines(t(N[,20:tmax]),lwd=2)
dev.off()

pdf(file="TimePlot_BGindep_exponential_log.pdf",width=12,height=8)
par(cex=1.2)
...
max_modulusEigen_DImodel=function(params, factorCorrect = 1 ){
  ...
  VR_prey = matrix(c((1-params[5])*params[1]*factorCorrect, params[7], 
                     params[5]*params[1]*factorCorrect,  params[2]),nrow = 2, ncol = 2)
  
  VR_pred =  matrix(c((1-params[6])*params[3], params[8]*factorCorrect, 
                      params[6]*params[3], params[4]),nrow = 2, ncol = 2)
