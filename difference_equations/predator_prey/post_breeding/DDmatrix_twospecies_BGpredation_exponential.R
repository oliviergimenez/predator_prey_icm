...
f=rep(NA,4) ### functions
f[1] = exp(-lambda[1]*N[2])     #effects of adult prey on juvenile prey
f[2] = exp(-lambda[2]*N[4])     #effects of adult predators on prey juv survival 
f[3] = exp(-lambda[3]*N[4])     #effects of adult predators on juvenile predator survival
f[4] = 1 - exp(-lambda[4]*N[1]) #adult predator eats juvenile prey which enhances predator prod
...
b_P = 2 # 0.25 (albatross -- one chick every two year, hence 1 female in 4 years) to 7 (artic fox)
...
pdf(file="TimePlot_BGpredation_exponential.pdf",width=12,height=8)
par(cex=1.2)
matplot(t(N[,20:tmax]),xlab="Time",ylab="Densities")
matlines(t(N[,20:tmax]),lwd=2)
dev.off()

pdf(file="TimePlot_BGpredation_exponential_log.pdf",width=12,height=8)
par(cex=1.2)
