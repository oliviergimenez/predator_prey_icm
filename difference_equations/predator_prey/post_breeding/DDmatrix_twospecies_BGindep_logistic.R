...
f=rep(NA,4)
### for (i in 1:4){f[i] = logistic(N[i],beta[i],eta[i])} ### This was completely wrong... 
...
pdf(file="TimePlot_BGindep_logistic.pdf",width=12,height=8)
par(cex=1.2)
matplot(t(N[,20:tmax]),xlab="Time",ylab="Densities")
matlines(t(N[,20:tmax]),lwd=2)
dev.off()

pdf(file="TimePlot_BGindep_logistic_log.pdf",width=12,height=8)
par(cex=1.2)
