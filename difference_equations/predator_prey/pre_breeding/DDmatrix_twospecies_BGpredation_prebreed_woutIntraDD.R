...
alpha[7] <- 1.75 #Here 2 shows easily the blowup, 1.5 takes up too much time
alpha[8] <- 0 # slopes
...
pdf(file="TimePlot_BGpredation_prebreed_woutIntraDD.pdf",width=12,height=8)
par(cex=1.2)
matplot(t(N[,1:tmax]),xlab="Time",ylab="Densities")
matlines(t(N[,1:tmax]),lwd=2)
dev.off()

pdf(file="Log_TimePlot_BGpredation_prebreed_woutIntraDD.pdf",width=12,height=8)
par(cex=1.2)
