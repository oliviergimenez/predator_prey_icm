...
pdf(file="TimePlot_BGpredation_prebreed_stochastic_newparam_highfec.pdf",width=12,height=8)
par(cex=1.2)
matplot(t(N[,nplotstart:nplotend]),xlab="Time",ylab="Densities")
matlines(t(N[,nplotstart:nplotend]),lwd=2)
dev.off()

pdf(file="Log_TimePlot_BGpredation_prebreed_stochastic_newparam_highfec_log.pdf",width=12,height=8)
par(cex=1.2)
matplot(t(log(N[,nplotstart:nplotend])),xlab="Time",ylab="log(Densities)")
